...
const Dashboard: FC<IProps> = ({
  ...
  return (
    <main className='flex min-h-0 flex-1 flex-col'>
      <section className='mx-auto flex min-h-0 w-full max-w-6xl flex-1 flex-col px-24 py-40'>
        ...
        <section className='grid shrink-0 grid-cols-2 gap-12 py-24'>
          ...
          <Card stats={`${solveRate.toFixed(2)}%`} title='solve rate' isLoading={isStatsLoading} />
        </section>
        <section className='flex min-h-0 flex-1 flex-col gap-12 py-24'>
          <div className='flex shrink-0 items-center justify-between'>
            <Typography text='body' medium>
              Recent games
            </Typography>
          </div>
          <div className='min-h-[400px] min-w-0 flex-1 overflow-hidden rounded-2xl border border-border bg-card shadow-soft'>
            {isLoading ? (
              ...
              <div className='h-full min-h-0 overflow-y-auto'>
                ...
                {isCursor && (
                  <div className='w-full flex items-center justify-center my-24 px-12'>
                    <Button
                      disabled={loader}
                      variant='dark'
                      onClick={() => {
